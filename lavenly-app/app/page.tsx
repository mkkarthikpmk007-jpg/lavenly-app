...
export default function Home() {
  ...
  const handleDeleteAccount = async (accName: string) => {
    if (!confirm(`"${accName}" wallet-ah delete panna athula ulla records-um poyidum. Delete pannalaama mapla?`)) return;
    ...
    return (
      <div className={`h-screen flex items-center justify-center ${isDarkMode ? 'bg-[#0F172A]' : 'bg-[#F3F0FF]'}`}>
        <div className={`p-10 rounded-[50px] shadow-2xl w-full max-w-sm border ${isDarkMode ? 'bg-[#1E293B] border-gray-700' : 'bg-white'}`}>
          ...
          <div className="flex items-center my-6"><div className="flex-1 h-px bg-gray-500/20"></div><span className="px-3 text-gray-400 text-[10px] font-black uppercase">OR</span><div className="flex-1 h-px bg-gray-500/20"></div></div>
    ...
    return (
      <div className={`h-screen flex flex-col items-center justify-center ${isDarkMode ? 'bg-[#0F172A]' : 'bg-[#F3F0FF]'}`}>
        <div className="text-center mb-10">
          ...
          <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest mt-3 italic">Keyboard Input Enabled</p>
        </div>
  ...
  return (
    <main className={`flex h-screen p-4 gap-4 transition-all duration-500 ${isDarkMode ? 'bg-[#0F172A] text-white' : 'bg-[#F8F9FD] text-gray-800'}`}>
      <aside className={`w-72 rounded-[40px] p-8 flex flex-col border ${isDarkMode ? 'bg-[#1E293B] border-gray-800' : 'bg-white shadow-lg'}`}>
        ...
        <button onClick={() => supabase.auth.signOut().then(() => setUser(null))} className="mt-auto p-5 text-red-500 font-bold flex items-center justify-center gap-2 bg-red-500/5 rounded-3xl hover:bg-red-500/10"><LogOut size={18}/> Logout</button>
      </aside>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-[#0F172A]/90 backdrop-blur-md flex items-center justify-center z-50 p-6">
          <div className={`w-full max-w-md p-10 rounded-[50px] shadow-2xl ${isDarkMode ? 'bg-[#1E293B] border border-gray-700' : 'bg-white'}`}>
            ...
            <input value={name} onChange={e => setName(e.target.value)} placeholder="What was this for?" className="w-full p-5 mb-5 rounded-3xl bg-gray-500/10 outline-none font-black text-lg" />
            ...
            <button onClick={handleSave} className="w-full bg-purple-600 text-white p-5 rounded-3xl font-black">CONFIRM LOG</button>
      ...
      {isAccModalOpen && (
        <div className="fixed inset-0 bg-[#0F172A]/90 backdrop-blur-md flex items-center justify-center z-50 p-6">
          <div className={`w-full max-w-md p-10 rounded-[50px] shadow-2xl ${isDarkMode ? 'bg-[#1E293B] border border-gray-700' : 'bg-white'}`}>
            ...
            <button onClick={() => setIsAccModalOpen(false)} className="w-full mt-4 text-gray-500 font-bold text-sm">Cancel</button>
          </div>
        </div>
      )}
    </main>
