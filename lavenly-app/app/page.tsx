...
export default function Home() {
  ...
  // Mock chart data - ithu ippo side-la theriyum
  const chartData = [
  ...
  return (
    <main className="flex min-h-screen bg-[#F3F0FF] p-4 gap-4 font-sans overflow-hidden">
      ...
      <aside className="w-72 bg-white rounded-[32px] p-6 flex flex-col shadow-sm border border-purple-50">
        ...
        <p className="text-gray-400 text-[10px] font-bold uppercase mb-4 px-2">My Accounts</p>
        <div className="flex flex-col gap-3 mb-8">
          {accounts.map(acc => (
            <div key={acc.id} className={`${acc.color} p-4 rounded-2xl text-white shadow-md transform hover:scale-105 transition-all cursor-pointer`}>
              <div className="flex justify-between items-start mb-2"><CreditCard size={16} className="opacity-80"/><span className="text-[10px] font-bold opacity-60 uppercase">Debit</span></div>
      ...
      <div className="flex-1 flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar">
        <header className="flex justify-between items-center bg-white p-6 rounded-[32px] shadow-sm shrink-0">
          <h2 className="text-2xl font-bold text-gray-800 tracking-tight">Welcome, Mapla!</h2>
        ...
        {/* Row 1: Balance & Chart side-by-side */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 shrink-0">
          <div className="bg-[#7C3AED] p-8 rounded-[32px] shadow-xl text-white relative overflow-hidden flex flex-col justify-center min-h-[200px]">
          ...
          <div className="bg-white p-6 rounded-[32px] shadow-sm border border-purple-50 min-h-[200px]">
            ...
            <div className="h-[120px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <Bar dataKey="value" radius={[6, 6, 6, 6]} barSize={25}>
        ...
        {/* Recent Transactions */}
        <div className="bg-white p-8 rounded-[32px] shadow-sm flex-1 border border-purple-50 mb-4">
