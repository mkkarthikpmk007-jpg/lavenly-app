...
export default function Home() {
  ...
  const handleAddAccount = async () => {
    ...
    const { error } = await supabase.from('accounts').insert([{ 
      name: accName, balance: parseFloat(accBalance), user_id: user.id 
    }]).select();
      ...
      <div className={`h-screen flex items-center justify-center ${isDarkMode ? 'bg-[#0F172A]' : 'bg-[#F3F0FF]'}`}>
        <div className={`p-10 rounded-[40px] shadow-2xl w-full max-w-sm border ${isDarkMode ? 'bg-[#1E293B] border-gray-700' : 'bg-white'}`}>
          ...
          <form onSubmit={handleLogin}>
            <input type="email" placeholder="Email" className="w-full p-4 mb-4 rounded-2xl bg-gray-100 border-none outline-none text-gray-800" value={email} onChange={e => setEmail(e.target.value)} />
            <input type="password" placeholder="Password" className="w-full p-4 mb-6 rounded-2xl bg-gray-100 border-none outline-none text-gray-800" value={password} onChange={e => setPassword(e.target.value)} />
    ...
    <main onKeyDown={(e) => e.key === 'Enter' && (isModalOpen ? handleSave() : isAccModalOpen ? handleAddAccount() : null)} className={`flex h-screen p-4 gap-4 ${isDarkMode ? 'bg-[#0F172A] text-white' : 'bg-[#F8F9FD] text-gray-800'}`}>
      ...
      <div className="flex-1 flex flex-col gap-6 overflow-y-auto pr-2">
        ...
        <div className={`p-8 rounded-[32px] flex-1 border ${isDarkMode ? 'bg-[#1E293B] border-gray-800' : 'bg-white shadow-sm'}`}>
          <h3 className="font-black text-xs uppercase opacity-40 mb-6 tracking-widest">Recent Records</h3>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className={`w-full max-w-sm p-8 rounded-[40px] shadow-2xl ${isDarkMode ? 'bg-[#1E293B] border-gray-700' : 'bg-white'}`}>
            ...
            <button onClick={handleSave} className="w-full bg-purple-600 text-white p-5 rounded-2xl font-black shadow-xl">CONFIRM LOG</button>
